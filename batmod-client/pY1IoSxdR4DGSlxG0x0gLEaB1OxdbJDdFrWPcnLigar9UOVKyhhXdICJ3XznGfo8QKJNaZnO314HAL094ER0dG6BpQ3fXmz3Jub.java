import java.util.HashMap;

public class pY1IoSxdR4DGSlxG0x0gLEaB1OxdbJDdFrWPcnLigar9UOVKyhhXdICJ3XznGfo8QKJNaZnO314HAL094ER0dG6BpQ3fXmz3Jub {
   public static void BatModClient(
      dYRU8kZZi9iCpYQ5cTFEJaOYxItWxAoHj02DIqn4ES0X0sIZCbd2emwajjqvW8TMMCUz6wNmhhl1AN75TGrnvdi963hCekgbLQrK var0,
      jbxQDpKgBIWhE7pHauqynFQUvqYDUW7OzHk8ctp7Peb6M6NLNn5fDAVGaQZ4c0p8H8lwFv3PPamtrehe3GqDwAfLrtdMNVCHOgsZ var1
   ) {
      try {
         fyZHvAc42MB0Bu0J2ogCjNyWdEgSTnpzdP2jvMzOaPsju8nPffPRDzLwKSnmA6lzZri3iNZGNWrbINOrUPalHNlLqHUVU6Zoy5hx var2 = xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.hSMlYe7503LeA7CbbVALz1jC9M8HB8zgNQY4LHkAnfyLgjvXzBsumpLcUY5Rcsl1recYw4lCY6oIVOA5xVfKlGQzbM1m6WJ9Zuae();
         if (var2 == null) {
            return;
         }

         if (!var2.bSALqmlxUoCEmKEx0a3lwGaLLejprOUkOjofxF0C0KpuhqUxhXAQfVMVMmCuFk2RMebQ12SqEyYOMBErHDyC9L41L7WH5mqOh2XJ) {
            return;
         }

         Throwable var3 = var0.Button();
         if (var3 == null) {
            return;
         }

         if (var3.getClass() == Throwable.class) {
            return;
         }

         if (var3.getClass().getName().contains(".fml.client.SplashProgress")) {
            return;
         }

         BatModClient(var1);
         String var4 = "http://optifine.net/crashReport";
         String var5 = BatModClient(var0);
         byte[] var6 = var5.getBytes("ASCII");
         qLYjScvzEj68wyxhFruPmPj2T0cbTeZLBret0nHCe8woM7zd9C5oiLRwAr1ICyjCbAwb3dVDCDoR6Rrl3kgOrqoRN9ZwmJ8WRtIU var7 = new qLYjScvzEj68wyxhFruPmPj2T0cbTeZLBret0nHCe8woM7zd9C5oiLRwAr1ICyjCbAwb3dVDCDoR6Rrl3kgOrqoRN9ZwmJ8WRtIU();
         HashMap var8 = new HashMap();
         var8.put(
            "OF-Version",
            xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.BatModClient()
         );
         var8.put("OF-Summary", Button(var0));
         lavrK51UM6S01MTZt0FYbazfhVRqJyxnyNdMwUJDZuHNpGaXTxWeRbep9FgQHfQxLu2ouhdBfHwMl4o2eJ7YvCLgNi4VRsT6yfYi var9 = new lavrK51UM6S01MTZt0FYbazfhVRqJyxnyNdMwUJDZuHNpGaXTxWeRbep9FgQHfQxLu2ouhdBfHwMl4o2eJ7YvCLgNi4VRsT6yfYi(
            var4, var8, var6, var7
         );
         var9.setPriority(10);
         var9.start();
         Thread.sleep(1000L);
      } catch (Exception var10) {
         xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.BatModClient(
            var10.getClass().getName() + ": " + var10.getMessage()
         );
      }
   }

   private static String BatModClient(
      dYRU8kZZi9iCpYQ5cTFEJaOYxItWxAoHj02DIqn4ES0X0sIZCbd2emwajjqvW8TMMCUz6wNmhhl1AN75TGrnvdi963hCekgbLQrK var0
   ) {
      StringBuffer var1 = new StringBuffer();
      var1.append(
         "OptiFineVersion: "
            + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.BatModClient()
            + "\n"
      );
      var1.append("Summary: " + Button(var0) + "\n");
      var1.append("\n");
      var1.append(var0.ButtonAction());
      var1.append("\n");
      return var1.toString();
   }

   private static String Button(
      dYRU8kZZi9iCpYQ5cTFEJaOYxItWxAoHj02DIqn4ES0X0sIZCbd2emwajjqvW8TMMCUz6wNmhhl1AN75TGrnvdi963hCekgbLQrK var0
   ) {
      Throwable var1 = var0.Button();
      if (var1 == null) {
         return "Unknown";
      }

      StackTraceElement[] var2 = var1.getStackTrace();
      String var3 = "unknown";
      if (var2.length > 0) {
         var3 = var2[0].toString().trim();
      }

      return var1.getClass().getName()
         + ": "
         + var1.getMessage()
         + " ("
         + var0.BatModClient()
         + ")"
         + " ["
         + var3
         + "]";
   }

   public static void BatModClient(
      jbxQDpKgBIWhE7pHauqynFQUvqYDUW7OzHk8ctp7Peb6M6NLNn5fDAVGaQZ4c0p8H8lwFv3PPamtrehe3GqDwAfLrtdMNVCHOgsZ var0
   ) {
      var0.BatModClient(
         "OptiFine Version",
         xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.BatModClient()
      );
      if (xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.hSMlYe7503LeA7CbbVALz1jC9M8HB8zgNQY4LHkAnfyLgjvXzBsumpLcUY5Rcsl1recYw4lCY6oIVOA5xVfKlGQzbM1m6WJ9Zuae()
         != null) {
         var0.BatModClient(
            "Render Distance Chunks",
            ""
               + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.PHC3PfXHfxNGhxr8qlmbwBgtFDAWis9MAjn8V7e7oXLSmezow4dZwTzrRYSwXzSWMtUii0JBu08E1JkvATKm8VH3vWAEeRNxaDVG()
         );
         var0.BatModClient(
            "Mipmaps",
            ""
               + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.LU8Huv8dNJpML2JB8fJWzj9J43KolrD8eyRsYcSdQZ3KH8jshbGfdwvq3RpIPDuukZZv21DROpftzYXoq05vVeGAMsPHlkNSkgtN()
         );
         var0.BatModClient(
            "Anisotropic Filtering",
            ""
               + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.SfmuZc1SobSZlHKCJucMVLA3DNMH6XACv7sE1WtGIL3Xu4YxuDOLGGkf37LRd8VzPhqiGdzutSvpKXc8AQwyZCDLNzOun6VymdGj()
         );
         var0.BatModClient(
            "Antialiasing",
            ""
               + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.QsmhBt00sCV5kzNmK6yKQdj9jyeR6rIC0yljgLPfF6buNd5njBBKCTOxq1qDZ2dV38nVTFCda6KTBv9jWLeXYGjlb5neCRT7ZbR()
         );
         var0.BatModClient(
            "Multitexture",
            ""
               + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.nuWP3Bei4rUZ6VCX2iWCHlHKiAsicTC9Ae69Jpwv4oZQMYPCTjiB0SikRnqAxZeaL93OS1vPuNC116DLDmI1tI2KbykrrH2Hg5n()
         );
      }

      var0.BatModClient(
         "Shaders",
         ""
            + shadersmod.client.mT4H8qpjr3K6mhbf9Pam0AP2YP1sTYGnvPD3wfHgfFZVvGjpI2ZdBsqiFfs9Rv0oFLlyx4nZpP6TITqRmJak428iQhhocwQiV8qv.QsmhBt00sCV5kzNmK6yKQdj9jyeR6rIC0yljgLPfF6buNd5njBBKCTOxq1qDZ2dV38nVTFCda6KTBv9jWLeXYGjlb5neCRT7ZbR()
      );
      var0.BatModClient(
         "OpenGlVersion",
         ""
            + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.Checkbox
      );
      var0.BatModClient(
         "OpenGlRenderer",
         ""
            + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.ProgressBar
      );
      var0.BatModClient(
         "OpenGlVendor",
         ""
            + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.BatModProgressBar
      );
      var0.BatModClient(
         "CpuCount",
         ""
            + xBYa9uNWziOSJdNzZI2TTXICLd4Bpw2tJDSeuOZv1cZbtJlLnXpVpRYSf6QHMBs0ZNE3u1OYk9qvdsAbuC49UFImqyyXHvohKe0q.b3NHK0RgJKZw5fPcxkySBhTqIQ8bauZtj1IlJTbVF5f8A3KPbUlb5bXIUzQO52rVdzAw8tHzzrx8s8Z1WzMQAUU0SQD8AmyN4AKX()
      );
   }
}
