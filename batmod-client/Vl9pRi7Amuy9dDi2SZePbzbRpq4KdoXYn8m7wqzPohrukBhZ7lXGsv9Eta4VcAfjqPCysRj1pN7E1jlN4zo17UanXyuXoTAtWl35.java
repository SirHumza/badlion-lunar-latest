import java.util.Random;

public class Vl9pRi7Amuy9dDi2SZePbzbRpq4KdoXYn8m7wqzPohrukBhZ7lXGsv9Eta4VcAfjqPCysRj1pN7E1jlN4zo17UanXyuXoTAtWl35
   extends jkYi7hkCQaHBMkKA7QEQmtjUnlsH2cwUoAUHHWA1iNnWc8aejyZGqr8KVgznAymE5pM3UlsAlWeHvpsePpzP3IztKwj3f6Kz4f5L {
   private float[] ButtonAction = new float[1024];

   protected void BatModClient(
      long var1,
      int var3,
      int var4,
      Z7u3kXmE1Z1La5wuW43LvM3MqXi2h2heVDrb5yErQvVFes5dvNjvuTD4vtNVaxJ74kxMHODIIL6qJ5Q6XSDm9TINzbpL91BffMM7 var5,
      double var6,
      double var8,
      double var10,
      float var12,
      float var13,
      float var14,
      int var15,
      int var16,
      double var17
   ) {
      Random var19 = new Random(var1);
      double var20 = var3 * 16 + 8;
      double var22 = var4 * 16 + 8;
      float var24 = 0.0F;
      float var25 = 0.0F;
      if (var16 <= 0) {
         int var26 = this.BatModClient * 16 - 16;
         var16 = var26 - var19.nextInt(var26 / 4);
      }

      boolean var64 = false;
      if (var15 == -1) {
         var15 = var16 / 2;
         var64 = true;
      }

      float var27 = 1.0F;

      for (int var28 = 0; var28 < 256; var28++) {
         if (var28 == 0 || var19.nextInt(3) == 0) {
            var27 = 1.0F + var19.nextFloat() * var19.nextFloat() * 1.0F;
         }

         this.ButtonAction[var28] = var27 * var27;
      }

      for (; var15 < var16; var15++) {
         double var65 = 1.5
            + u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.BatModClient(
                  var15 * (float) Math.PI / var16
               )
               * var12
               * 1.0F;
         double var30 = var65 * var17;
         var65 *= var19.nextFloat() * 0.25 + 0.75;
         var30 *= var19.nextFloat() * 0.25 + 0.75;
         float var32 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.Button(
            var14
         );
         float var33 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.BatModClient(
            var14
         );
         var6 += u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.Button(
               var13
            )
            * var32;
         var8 += var33;
         var10 += u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.BatModClient(
               var13
            )
            * var32;
         var14 *= 0.7F;
         var14 += var25 * 0.05F;
         var13 += var24 * 0.05F;
         var25 *= 0.8F;
         var24 *= 0.5F;
         var25 += (var19.nextFloat() - var19.nextFloat()) * var19.nextFloat() * 2.0F;
         var24 += (var19.nextFloat() - var19.nextFloat()) * var19.nextFloat() * 4.0F;
         if (var64 || var19.nextInt(4) != 0) {
            double var34 = var6 - var20;
            double var36 = var10 - var22;
            double var38 = var16 - var15;
            double var40 = var12 + 2.0F + 16.0F;
            if (var34 * var34 + var36 * var36 - var38 * var38 > var40 * var40) {
               return;
            }

            if (var6 >= var20 - 16.0 - var65 * 2.0
               && var10 >= var22 - 16.0 - var65 * 2.0
               && var6 <= var20 + 16.0 + var65 * 2.0
               && var10 <= var22 + 16.0 + var65 * 2.0) {
               int var42 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var6 - var65
                  )
                  - var3 * 16
                  - 1;
               int var43 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var6 + var65
                  )
                  - var3 * 16
                  + 1;
               int var44 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var8 - var30
                  )
                  - 1;
               int var45 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var8 + var30
                  )
                  + 1;
               int var46 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var10 - var65
                  )
                  - var4 * 16
                  - 1;
               int var47 = u4L2raCRW4IaxQDTMTkLSzlPPuZcdlIz0ZOemtSZ590eQlF5phhjcz9AjgDml3JLkKDI91FuMRnQzu4nUzDxPrtX19Vazyl9vOfr.CustomSpinner(
                     var10 + var65
                  )
                  - var4 * 16
                  + 1;
               if (var42 < 0) {
                  var42 = 0;
               }

               if (var43 > 16) {
                  var43 = 16;
               }

               if (var44 < 1) {
                  var44 = 1;
               }

               if (var45 > 248) {
                  var45 = 248;
               }

               if (var46 < 0) {
                  var46 = 0;
               }

               if (var47 > 16) {
                  var47 = 16;
               }

               boolean var48 = false;

               for (int var49 = var42; !var48 && var49 < var43; var49++) {
                  for (int var50 = var46; !var48 && var50 < var47; var50++) {
                     for (int var51 = var45 + 1; !var48 && var51 >= var44 - 1; var51--) {
                        if (var51 >= 0 && var51 < 256) {
                           jfbpd0kp1QEJJMd5cKOo0MgyHaqrgeaYoTLTse3y0ldrAiXTN7OrPJfu5yBfixhCqmdWDYWOZidiavQVW5Qe7WjmXy5eN8CC8fSS var52 = var5.BatModClient(
                              var49, var51, var50
                           );
                           if (var52.CustomSpinner()
                                 == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.ColorChooser
                              || var52.CustomSpinner()
                                 == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.IntegerSpinner
                              )
                            {
                              var48 = true;
                           }

                           if (var51 != var44 - 1 && var49 != var42 && var49 != var43 - 1 && var50 != var46 && var50 != var47 - 1) {
                              var51 = var44;
                           }
                        }
                     }
                  }
               }

               if (!var48) {
                  ylSfcWKRkKAdTzwagatDMI4oqlbFRATxQqPDJvnC4xsn2r5CgY7jERHfJfBxgspEAOCvnOSkRVNZgXroLW2VBLJuYJlwrHgVPIyS var68 = new ylSfcWKRkKAdTzwagatDMI4oqlbFRATxQqPDJvnC4xsn2r5CgY7jERHfJfBxgspEAOCvnOSkRVNZgXroLW2VBLJuYJlwrHgVPIyS();

                  for (int var69 = var42; var69 < var43; var69++) {
                     double var70 = (var69 + var3 * 16 + 0.5 - var6) / var65;

                     for (int var53 = var46; var53 < var47; var53++) {
                        double var54 = (var53 + var4 * 16 + 0.5 - var10) / var65;
                        boolean var56 = false;
                        if (var70 * var70 + var54 * var54 < 1.0) {
                           for (int var57 = var45; var57 > var44; var57--) {
                              double var58 = (var57 - 1 + 0.5 - var8) / var30;
                              if ((var70 * var70 + var54 * var54)
                                       * this.ButtonAction[var57 - 1]
                                    + var58 * var58 / 6.0
                                 < 1.0) {
                                 jfbpd0kp1QEJJMd5cKOo0MgyHaqrgeaYoTLTse3y0ldrAiXTN7OrPJfu5yBfixhCqmdWDYWOZidiavQVW5Qe7WjmXy5eN8CC8fSS var60 = var5.BatModClient(
                                    var69, var57, var53
                                 );
                                 if (var60.CustomSpinner()
                                    == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.CustomSpinner
                                    )
                                  {
                                    var56 = true;
                                 }

                                 if (var60.CustomSpinner()
                                       == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.Button
                                    || var60.CustomSpinner()
                                       == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.ButtonAction
                                    || var60.CustomSpinner()
                                       == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.CustomSpinner
                                    )
                                  {
                                    if (var57 - 1 < 10) {
                                       var5.BatModClient(
                                          var69,
                                          var57,
                                          var53,
                                          fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.TextField
                                             .OXiHn4yUSlOUP4kAV8kog6OFjXxn0PRuO4COz26Lq0ilLnotGRRNuaBnh7uhZgmQElMCcIzA31DVhxqMomb5cN7KXGc0anrqbaJt()
                                       );
                                    } else {
                                       var5.BatModClient(
                                          var69,
                                          var57,
                                          var53,
                                          fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.BatModClient
                                             .OXiHn4yUSlOUP4kAV8kog6OFjXxn0PRuO4COz26Lq0ilLnotGRRNuaBnh7uhZgmQElMCcIzA31DVhxqMomb5cN7KXGc0anrqbaJt()
                                       );
                                       if (var56
                                          && var5.BatModClient(
                                                   var69, var57 - 1, var53
                                                )
                                                .CustomSpinner()
                                             == fjTmmqSsZED0Quce5L2uiRkXlUvTxobqZQQLKtQCEfpkCpo2EBl8Znhs2ME7pKps4B0QFBco0DiLYL8M5rwIsjvXbiKu69mgXWo.ButtonAction
                                          )
                                        {
                                          var68.Button(
                                             var69 + var3 * 16, 0, var53 + var4 * 16
                                          );
                                          var5.BatModClient(
                                             var69,
                                             var57 - 1,
                                             var53,
                                             this.CustomSpinner
                                                .CustomSpinner(var68)
                                                .reBVl7yfwB6jG1tjPNyF5dLNOL952zqPCpbgvinOK5i60q4az9065mMTV6lTqaa7To8zXm5MC4ThCJhwvxhorIlHXClWOr1Ec3Hz
                                          );
                                       }
                                    }
                                 }
                              }
                           }
                        }
                     }
                  }

                  if (var64) {
                     break;
                  }
               }
            }
         }
      }
   }

   @Override
   protected void BatModClient(
      xDjs4YCZLUvHVJGII20owVT8rXpk3nSu1jLsjzeLtjo7r8lq7RYaHZp1O5oLhmXPyqOR2Eev73tmbyPF4I049uP0lj5ZD5blgUPY var1,
      int var2,
      int var3,
      int var4,
      int var5,
      Z7u3kXmE1Z1La5wuW43LvM3MqXi2h2heVDrb5yErQvVFes5dvNjvuTD4vtNVaxJ74kxMHODIIL6qJ5Q6XSDm9TINzbpL91BffMM7 var6
   ) {
      if (this.Button.nextInt(50) == 0) {
         double var7 = var2 * 16 + this.Button.nextInt(16);
         double var9 = this.Button
               .nextInt(this.Button.nextInt(40) + 8)
            + 20;
         double var11 = var3 * 16 + this.Button.nextInt(16);
         byte var13 = 1;

         for (int var14 = 0; var14 < var13; var14++) {
            float var15 = this.Button.nextFloat()
               * (float) Math.PI
               * 2.0F;
            float var16 = (this.Button.nextFloat() - 0.5F)
               * 2.0F
               / 8.0F;
            float var17 = (
                  this.Button.nextFloat() * 2.0F
                     + this.Button.nextFloat()
               )
               * 2.0F;
            this.BatModClient(
               this.Button.nextLong(),
               var4,
               var5,
               var6,
               var7,
               var9,
               var11,
               var17,
               var15,
               var16,
               0,
               0,
               3.0
            );
         }
      }
   }
}
