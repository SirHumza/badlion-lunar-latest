      ...
      while (true) {
         ...
         while (true) {
            ...
            if (back < 65536 && LZ4UnsafeUtils.readIntEquals(src, ref, sOff)) {
               h = LZ4UnsafeUtils.commonBytesBackward(src, ref, sOff, srcOff, anchor);
               sOff -= h;
               ref -= h;
               int runLen = sOff - anchor;
               int tokenOff = dOff++;
               ...
               while (true) {
                  ...
                  int hx = LZ4Utils.hash(UnsafeUtils.readInt(src, sOff));
                  ref = UnsafeUtils.readInt(hashTable, hx);
                  UnsafeUtils.writeInt(hashTable, hx, sOff);
                  back = sOff - ref;
                  if (back >= 65536 || !LZ4UnsafeUtils.readIntEquals(src, ref, sOff)) {
                     anchor = sOff++;
                     continue label63;
