...
public class LZ4BlockInputStream extends FilterInputStream {
   ...
   public int read() throws IOException {
      ...
      return this.finished ? -1 : this.buffer[this.o++] & 0xFF;
   }
   ...
   private void refill() throws IOException {
      if (!this.tryReadFully(this.compressedBuffer, LZ4BlockOutputStream.HEADER_LENGTH)) {
         ...
         if (this.originalLen <= 1 << compressionLevel
            && this.originalLen >= 0
            && compressedLen >= 0
            && (this.originalLen != 0 || compressedLen == 0)
            && (this.originalLen == 0 || compressedLen != 0)
            && (compressionMethod != 16 || this.originalLen == compressedLen)) {
