...
const registerStreamsMessageBroker = ({ launchApp, ipcMain, logger, mainWindow, chatWindow, }) => {
    if (!launchApp)
        return;
    ...
    launchApp
        ...
        if (chatWindow && !chatWindow.isDestroyed() && chatWindow.webContents) {
            chatWindow.webContents.send('receive-streams-message', data);
        }
    })
        .then(callbackSetterResponseCode => {
...
const streamsRequest = ({ launchApp, logger, data, service, awaitResponse = true, }) => new Promise((resolve, reject) => {
    ...
    // Send the request
    launchApp.sendStreamsMessage(payload).then(requestResponseCode => {
        if (requestResponseCode === SUCCESS_CODE) {
            if (awaitResponse) {
                ...
                resolve({ success: true, messageId: streamsMessageId });
            }
        }
        else {
            // Request failed
...
const schema = {
    ...
    client_background_patching_behaviour: {
        type: 'string',
        default: 'off',
    },
    client_background_download_speed: {
        type: 'number',
        default: 100,
    },
    client_background_download_limit: {
        type: 'boolean',
        default: true,
    },
    custom_chat_bubbles_enabled: {
        type: 'boolean',
        default: true,
    },
};
...
logger__default['default'].debug('Electron main thread initializing...', {
    versions: {
        ...
        electron: process__default['default'].versions.electron,
        node: process__default['default'].versions.node,
        chrome: process__default['default'].versions.chrome,
    },
});
const isWindows = process__default['default'].platform === 'win32';
const isMac = process__default['default'].platform === 'darwin';
const isLinux = process__default['default'].platform === 'linux';
const MAIN_WINDOW_MIN_WIDTH = 1260;
const MAIN_WINDOW_MIN_HEIGHT = isMac ? 720 : 725;
const SESSION_NAME_CHAT_WINDOW = 'persist:session-chat-window';
...
const chatWindowSettings = {
    ...
    webPreferences: {
        ...
        preload: path__default['default'].join(__dirname, 'preload.js'),
        sandbox: false,
        partition: SESSION_NAME_CHAT_WINDOW,
    },
};
...
if (isLinux) {
    ...
    chatWindowSettings.icon = path__default['default'].join(__dirname, '64x64.png');
}
let opensMinimized = false;
let updaterFinished = false;
let chatWindowStateSavedOnQuit = false;
...
function asyncMain(loadError) {
    if (!loadError) {
        logger__default['default'].info('loaded launcher.node');
    }
    // This function sends a request to the child process to execute a native function
    async function callNativeFunction(functionName, args) {
        // logger.info('calling native function', functionName);
        return new Promise((resolve, reject) => {
            let callbackId = ++callbackIds;
            let functions = new Map();
            let functionsToSend = new Array();
            args.forEach((value, index) => {
                if (typeof value === 'function') {
                    functions.set(index, value);
                    functionsToSend.push(index);
                }
            });
            ...
            childProcess.send({ functionName, args, callbackId, functionsToSend });
        });
    }
    let launchApp;
    // @ts-ignore
            ...
            ? [
                {
                    ...
                    submenu: [
                        {
                            label: 'About Badlion Client',
                            click: () => {
                                if (mainWindow) {
                                    // Send an IPC event to show the custom about modal
    ...
    const showMainWindow = () => {
        ...
        if (!opensMinimized) {
            mainWindow.show();
            if (jsonDatabase.getDatabase()?.get('show_chat_window')) {
                showChatWindow();
            }
    ...
    const queueUpdaterMessage = (message, ...args) => {
        ...
        const oldTimer = updateMessageQueueTimers[message];
        if (oldTimer) {
     ...
     */
    electron.ipcMain.handle('launch-app-invoke-method', async (event, method, invocationHash, ...args) => {
        ...
        try {
            // Invoke the method
            return await launchApp[method].apply(null, argsParsed);
        }
        catch (e) {
    ...
    electron.ipcMain.handle('dispatch-auth-change-to-windows', (event, data) => {
        logger__default['default'].info('dispatch auth change to all windows', data);
        if (chatWindow && !chatWindow.isDestroyed()) {
            chatWindow.webContents.send('receive-auth-change', data);
        }
    });
    electron.ipcMain.handle('router-navigate', (event, route) => {
        if (mainWindow) {
            mainWindow.webContents.send('router-navigate-received', route);
        }
    });
        ...
        return new Promise((resolve, reject) => {
            fs__default['default'].readFile(path__default['default'].join(`${filePath}`, 'level.dat'), (err, data) => {
                ...
                if (!data || !isObjectLike__default['default'](data)) {
                    const result = 'One or more level.dat files are corrupt or empty. Path: ' +
                        filePath;
                    logger__default['default'].error(result);
                    return;
                }
    ...
    electron.app.whenReady().then(() => {
        electron.app.setAppUserModelId('net.badlion.client');
        const args = process__default['default'].argv.slice(1);
        ...
        electron.ipcMain.handle('webview-mouseup', (event, mouseButton) => {
            if (mainWindow && !mainWindow.isDestroyed() && mainWindow.webContents) {
                mainWindow.webContents.send('webview-mouseup-receive', mouseButton);
            }
        });
        // Within webviews prevent navigation to other websites conditionally.
        electron.app.on('web-contents-created', (event, contents) => {
            if (contents.getType() === 'webview') {
                contents.on('will-attach-webview', (event, webPreferences) => {
                    // https://www.electronjs.org/docs/latest/tutorial/security For webview security
                ...
                contents.on('will-navigate', (event, url) => {
                    const parsedUrl = new URL(url); // test against URL object instead of using startsWith on url string for more security.
                    if (parsedUrl.origin !== 'https://support.badlion.net') {
                        // Open externally in another browser if the origin of the url to navigate to is NOT https://support.badlion.net
                        event.preventDefault();
                        electron.shell.openExternal(parsedUrl.href);
                    }
                });
            }
        });
        ...
        createChatWindow();
        // Register streams message broker
        registerStreamsMessageBroker({
            launchApp,
    ...
    const sendMicrosoftOAuthFlowError = (errorCode) => {
        logger__default['default'].error('Microsoft Auth - Minecraft account error', errorCode);
    ...
    const sendNewMicrosoftOAuthFlowError = (errorCode) => {
        logger__default['default'].error('Microsoft Auth Badlion account error', errorCode);
        // Send to renderer that we killed it ourselves
        if (mainWindow) {
            const response = { err: errorCode };
            const data = JSON.stringify(response);
    ...
    electron.ipcMain.handle('backend-auth-state-complete', (event, success) => {
        logger__default['default'].debug('backend-auth-state-complete', success);
        if (mainWindow && mainWindow.webContents && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('backend-auth-state-complete', success);
        }
    });
    ...
    electron.ipcMain.handle('show-chat-window', (event, data) => {
        showChatWindow();
        if (data) {
            chatWindow?.webContents?.send('chat-window-set-view', data);
        }
    });
    electron.ipcMain.handle('minimize-chat-window', event => {
        if (chatWindow && !chatWindow.isDestroyed()) {
            chatWindow.minimize();
        }
    });
    electron.ipcMain.handle('hide-chat-window', event => {
        if (chatWindow && !chatWindow.isDestroyed()) {
            logger__default['default'].verbose('Hide chat window');
            chatWindow.hide();
        }
    });
    electron.ipcMain.handle('get-main-window-open', event => mainWindow && !mainWindow.isDestroyed() && mainWindow.isVisible());
    electron.ipcMain.handle('get-main-window-focused', event => mainWindow &&
        !mainWindow.isDestroyed() &&
        mainWindow.isVisible() &&
        mainWindow.isFocused());
    electron.ipcMain.handle('get-chat-window-open', event => chatWindow && !chatWindow.isDestroyed() && chatWindow.isVisible());
    electron.ipcMain.handle('get-chat-window-focused', event => chatWindow &&
        !chatWindow.isDestroyed() &&
        chatWindow.isVisible() &&
        chatWindow.isFocused());
    electron.ipcMain.handle('settings-update', (event, data) => {
        const targetWindow = event.sender.id === mainWindow?.webContents?.id ? chatWindow : mainWindow;
        if (targetWindow && !targetWindow.isDestroyed()) {
            targetWindow?.webContents?.send('settings-update', data);
        }
    });
    electron.ipcMain.handle('chat-settings-update', (event, data) => {
        const targetWindow = event.sender.id === mainWindow?.webContents?.id ? chatWindow : mainWindow;
        if (targetWindow && !targetWindow.isDestroyed()) {
            targetWindow?.webContents?.send('chat-settings-update', data);
        }
    });
    const showChatWindow = () => {
        ...
        if (!chatWindow) {
            logger__default['default'].error('Could not create chat window');
            return;
        }
        logger__default['default'].verbose('Show chat window');
    ...
    const createChatWindow = () => {
        if ((chatWindow && !chatWindow.isDestroyed()) || !mainWindow)
            return;
        chatWindow = createWindow('chat', chatWindowSettings);
        chatWindow.removeMenu();
        fixWebsocketCookiesOnSession(electron.session.fromPartition(SESSION_NAME_CHAT_WINDOW));
        chatWindow.loadFile(path__default['default'].join(__dirname, 'chat.html')).catch(e => {
            logger__default['default'].info('error loading url for chatWindow', e);
        });
        logger__default['default'].verbose('chat window url loaded.');
    };
    const saveChatWindowState = () => {
        if (!chatWindowStateSavedOnQuit && chatWindow) {
            logger__default['default'].silly('Save chat window state on quit');
            jsonDatabase
                .getDatabase()
                ?.set('show_chat_window', !chatWindow.isDestroyed() && chatWindow.isVisible());
            chatWindowStateSavedOnQuit = true;
        }
    };
}
childProcess = child_process.fork(path__default['default'].join(__dirname, 'child-process.js'), {
    execPath: process__default['default'].execPath,
});
logger__default['default'].info('child process started', childProcess);
// Wait for the child process to load the native module
childProcess.on('message', (msg) => {
    // logger.info('got message from child process', msg);
    switch (msg.type) {
        case 'node_child_process_load': {
        ...
        case 'node_child_process_error': {
            let callback = callbacks.get(msg.callback_id);
            if (callback) {
                callback.reject('native function not found');
            }
            return;
        }
        case 'node_child_process_return_value': {
            let callback = callbacks.get(msg.callback_id);
            if (callback) {
        ...
        case 'node_child_process_callback': {
            let callback = callbacks.get(msg.callback_id);
            if (callback) {
                callback.functions.get(msg.callback_index)(...msg.callback_args);
            }
            return;
        }
    }
...
childProcess.on('error', err => {
    logger__default['default'].error('child process error:', err);
});
childProcess.on('exit', (code, signal) => {
    logger__default['default'].error('child process exited:', code, signal);
    if (code !== null) {
...
// TODO: Cleanup callbacks to prevent memory leak. Although, finding out when to delete them is gonna be tricky. The IPC between main process and renderer doesn't even clean them up (see https://github.com/BadlionClient/premium-client/blob/fa10b307a2d57b1fb3608db82d3dded42cbe6b6d/native-desktop/src/preload.ts#L144)
