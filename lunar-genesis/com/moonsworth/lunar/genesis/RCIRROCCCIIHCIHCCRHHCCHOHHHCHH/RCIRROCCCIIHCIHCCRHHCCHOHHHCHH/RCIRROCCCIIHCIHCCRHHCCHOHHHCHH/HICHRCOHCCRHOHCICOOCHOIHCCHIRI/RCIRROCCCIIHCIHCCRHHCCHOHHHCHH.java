package com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HICHRCOHCCRHOHCICOOCHOIHCCHIRI;

import com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.IRCIIHHICIHRCOCRROCOICRIHHCCHH.IRCRRHRCIRHIHIHROHCRRHIIHHHHCH;
import java.math.BigDecimal;
import java.math.BigInteger;
import java.math.RoundingMode;
import java.util.ArrayList;
import java.util.List;

@com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.IRCIIHHICIHRCOCRROCOICRIHHCCHH(
   emulated = true
)
public final class RCIRROCCCIIHCIHCCRHHCCHOHHHCHH {
   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.CRRRICCRROCOHHOHIICIHORCOORRRH
   static final int RRRHCHOICICHHIHCCRORORRHCOCHIO = 256;
   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.CRRRICCRROCOHHOHIICIHORCOORRRH
   static final BigInteger ICRRHOOHRCIRIRHOOOCRIHRIIHRCCO = new BigInteger("16a09e667f3bcc908b2fb1366ea957d3e3adec17512775099da2f590b0667322a", 16);
   private static final double IIROHIRCCHCORIIRROOHRRHOOIRHIC = Math.log(10.0);
   private static final double IIRHORHHOIIRROHCCCROOHOIOHCHCO = Math.log(2.0);

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH
   public static BigInteger ceilingPowerOfTwo(BigInteger var0) {
      return BigInteger.ZERO.setBit(log2(var0, RoundingMode.CEILING));
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH
   public static BigInteger floorPowerOfTwo(BigInteger var0) {
      return BigInteger.ZERO.setBit(log2(var0, RoundingMode.FLOOR));
   }

   public static boolean isPowerOfTwo(BigInteger var0) {
      IRCRRHRCIRHIHIHROHCRRHIIHHHHCH.checkNotNull(var0);
      return var0.signum() > 0 && var0.getLowestSetBit() == var0.bitLength() - 1;
   }

   public static int log2(BigInteger var0, RoundingMode var1) {
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkPositive("x", IRCRRHRCIRHIHIHROHCRRHIIHHHHCH.checkNotNull(var0));
      int var2 = var0.bitLength() - 1;
      switch (var1) {
         case UNNECESSARY:
            HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkRoundingUnnecessary(isPowerOfTwo(var0));
         case DOWN:
         case FLOOR:
            return var2;
         case UP:
         case CEILING:
            return isPowerOfTwo(var0) ? var2 : var2 + 1;
         case HALF_DOWN:
         case HALF_UP:
         case HALF_EVEN:
            if (var2 < 256) {
               BigInteger var5 = ICRRHOOHRCIRIRHOOOCRIHRIIHRCCO.shiftRight(256 - var2);
               if (var0.compareTo(var5) <= 0) {
                  return var2;
               }

               return var2 + 1;
            }

            BigInteger var3 = var0.pow(2);
            int var4 = var3.bitLength() - 1;
            return var4 < 2 * var2 + 1 ? var2 : var2 + 1;
         default:
            throw new AssertionError();
      }
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   public static int log10(BigInteger var0, RoundingMode var1) {
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkPositive("x", var0);
      if (fitsInLong(var0)) {
         return RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.log10(var0.longValue(), var1);
      }

      int var2 = (int)(log2(var0, RoundingMode.FLOOR) * IIRHORHHOIIRROHCCCROOHOIOHCHCO / IIROHIRCCHCORIIRROOHRRHOOIRHIC);
      BigInteger var3 = BigInteger.TEN.pow(var2);
      int var4 = var3.compareTo(var0);
      if (var4 > 0) {
         do {
            var2--;
            var3 = var3.divide(BigInteger.TEN);
            var4 = var3.compareTo(var0);
         } while (var4 > 0);
      } else {
         BigInteger var5 = BigInteger.TEN.multiply(var3);

         for (int var6 = var5.compareTo(var0); var6 <= 0; var6 = var5.compareTo(var0)) {
            var2++;
            var3 = var5;
            var4 = var6;
            var5 = BigInteger.TEN.multiply(var3);
         }
      }

      int var10 = var2;
      BigInteger var11 = var3;
      int var7 = var4;
      switch (var1) {
         case UNNECESSARY:
            HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkRoundingUnnecessary(var7 == 0);
         case DOWN:
         case FLOOR:
            return var10;
         case UP:
         case CEILING:
            return var11.equals(var0) ? var10 : var10 + 1;
         case HALF_DOWN:
         case HALF_UP:
         case HALF_EVEN:
            BigInteger var8 = var0.pow(2);
            BigInteger var9 = var11.pow(2).multiply(BigInteger.TEN);
            return var8.compareTo(var9) <= 0 ? var10 : var10 + 1;
         default:
            throw new AssertionError();
      }
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   public static BigInteger sqrt(BigInteger var0, RoundingMode var1) {
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkNonNegative("x", var0);
      if (fitsInLong(var0)) {
         return BigInteger.valueOf(RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.sqrt(var0.longValue(), var1));
      }

      BigInteger var2 = sqrtFloor(var0);
      switch (var1) {
         case UNNECESSARY:
            HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkRoundingUnnecessary(var2.pow(2).equals(var0));
         case DOWN:
         case FLOOR:
            return var2;
         case UP:
         case CEILING:
            int var3 = var2.intValue();
            boolean var4 = var3 * var3 == var0.intValue() && var2.pow(2).equals(var0);
            return var4 ? var2 : var2.add(BigInteger.ONE);
         case HALF_DOWN:
         case HALF_UP:
         case HALF_EVEN:
            BigInteger var5 = var2.pow(2).add(var2);
            return var5.compareTo(var0) >= 0 ? var2 : var2.add(BigInteger.ONE);
         default:
            throw new AssertionError();
      }
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   private static BigInteger sqrtFloor(BigInteger var0) {
      int var2 = log2(var0, RoundingMode.FLOOR);
      BigInteger var1;
      if (var2 < 1023) {
         var1 = sqrtApproxWithDoubles(var0);
      } else {
         int var3 = var2 - 52 & -2;
         var1 = sqrtApproxWithDoubles(var0.shiftRight(var3)).shiftLeft(var3 >> 1);
      }

      BigInteger var5 = var1.add(var0.divide(var1)).shiftRight(1);
      if (var1.equals(var5)) {
         return var1;
      }

      do {
         var1 = var5;
         var5 = var1.add(var0.divide(var1)).shiftRight(1);
      } while (var5.compareTo(var1) < 0);

      return var1;
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   private static BigInteger sqrtApproxWithDoubles(BigInteger var0) {
      return IRCIIHHICIHRCOCRROCOICRIHHCCHH.roundToBigInteger(Math.sqrt(HHCCIRHCCCIIRHCROHIORHIRHHIORH.bigToDouble(var0)), RoundingMode.HALF_EVEN);
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   public static BigInteger divide(BigInteger var0, BigInteger var1, RoundingMode var2) {
      BigDecimal var3 = new BigDecimal(var0);
      BigDecimal var4 = new BigDecimal(var1);
      return var3.divide(var4, 0, var2).toBigIntegerExact();
   }

   public static BigInteger factorial(int var0) {
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkNonNegative("n", var0);
      if (var0 < RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.IORIHCCCOHOCOHRIOHIIORCHCRHIRO.length) {
         return BigInteger.valueOf(RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.IORIHCCCOHOCOHRIOHIIORCHCRHIRO[var0]);
      }

      int var1 = CRRRICCRROCOHHOHIICIHORCOORRRH.divide(var0 * CRRRICCRROCOHHOHIICIHORCOORRRH.log2(var0, RoundingMode.CEILING), 64, RoundingMode.CEILING);
      ArrayList var2 = new ArrayList(var1);
      int var3 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.IORIHCCCOHOCOHRIOHIIORCHCRHIRO.length;
      long var4 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.IORIHCCCOHOCOHRIOHIIORCHCRHIRO[var3 - 1];
      int var6 = Long.numberOfTrailingZeros(var4);
      var4 >>= var6;
      int var7 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.log2(var4, RoundingMode.FLOOR) + 1;
      int var8 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.log2(var3, RoundingMode.FLOOR) + 1;
      int var9 = 1 << var8 - 1;

      for (long var10 = var3; var10 <= var0; var10++) {
         if ((var10 & var9) != 0L) {
            var9 <<= 1;
            var8++;
         }

         int var12 = Long.numberOfTrailingZeros(var10);
         long var13 = var10 >> var12;
         var6 += var12;
         int var15 = var8 - var12;
         if (var15 + var7 >= 64) {
            var2.add(BigInteger.valueOf(var4));
            var4 = 1L;
            boolean var17 = false;
         }

         var4 *= var13;
         var7 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.log2(var4, RoundingMode.FLOOR) + 1;
      }

      if (var4 > 1L) {
         var2.add(BigInteger.valueOf(var4));
      }

      return listProduct(var2).shiftLeft(var6);
   }

   static BigInteger listProduct(List<BigInteger> var0) {
      return listProduct(var0, 0, var0.size());
   }

   static BigInteger listProduct(List<BigInteger> var0, int var1, int var2) {
      switch (var2 - var1) {
         case 0:
            return BigInteger.ONE;
         case 1:
            return (BigInteger)var0.get(var1);
         case 2:
            return ((BigInteger)var0.get(var1)).multiply((BigInteger)var0.get(var1 + 1));
         case 3:
            return ((BigInteger)var0.get(var1)).multiply((BigInteger)var0.get(var1 + 1)).multiply((BigInteger)var0.get(var1 + 2));
         default:
            int var3 = var2 + var1 >>> 1;
            return listProduct(var0, var1, var3).multiply(listProduct(var0, var3, var2));
      }
   }

   public static BigInteger binomial(int var0, int var1) {
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkNonNegative("n", var0);
      HRCHROOHRIHCRCRHRIIROCIRHOIRHH.checkNonNegative("k", var1);
      IRCRRHRCIRHIHIHROHCRRHIIHHHHCH.checkArgument(var1 <= var0, "k (%s) > n (%s)", var1, var0);
      if (var1 > var0 >> 1) {
         var1 = var0 - var1;
      }

      if (var1 < RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.CRIIHIOCICCOICRHHRCRCCRRHIHCRI.length
         && var0 <= RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.CRIIHIOCICCOICRHHRCRCCRRHIHCRI[var1]) {
         return BigInteger.valueOf(RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.binomial(var0, var1));
      }

      BigInteger var2 = BigInteger.ONE;
      long var3 = var0;
      long var5 = 1L;
      int var7 = RIOOCHICIHRHOHCCCCCHOCCCOHCRHI.log2(var0, RoundingMode.CEILING);
      int var8 = var7;

      for (int var9 = 1; var9 < var1; var9++) {
         int var10 = var0 - var9;
         int var11 = var9 + 1;
         if (var8 + var7 >= 63) {
            var2 = var2.multiply(BigInteger.valueOf(var3)).divide(BigInteger.valueOf(var5));
            var3 = var10;
            var5 = var11;
            var8 = var7;
         } else {
            var3 *= var10;
            var5 *= var11;
            var8 += var7;
         }
      }

      return var2.multiply(BigInteger.valueOf(var3)).divide(BigInteger.valueOf(var5));
   }

   @com.moonsworth.lunar.genesis.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.RCIRROCCCIIHCIHCCRHHCCHOHHHCHH.HHCCIRHCCCIIRHCROHIORHIRHHIORH
   static boolean fitsInLong(BigInteger var0) {
      return var0.bitLength() <= 63;
   }

   private RCIRROCCCIIHCIHCCRHHCCHOHHHCHH() {
   }
}
