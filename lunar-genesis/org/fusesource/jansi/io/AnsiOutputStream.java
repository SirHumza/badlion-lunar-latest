package org.fusesource.jansi.io;

import java.io.FilterOutputStream;
import java.io.OutputStream;
import java.nio.charset.Charset;
import java.util.ArrayList;
import org.fusesource.jansi.AnsiColors;
import org.fusesource.jansi.AnsiMode;
import org.fusesource.jansi.AnsiType;

public class AnsiOutputStream extends FilterOutputStream {
   public static final byte[] RESET_CODE = "\u001b[0m".getBytes();
   private static final int LOOKING_FOR_FIRST_ESC_CHAR = 0;
   private static final int LOOKING_FOR_SECOND_ESC_CHAR = 1;
   private static final int LOOKING_FOR_NEXT_ARG = 2;
   private static final int LOOKING_FOR_STR_ARG_END = 3;
   private static final int LOOKING_FOR_INT_ARG_END = 4;
   private static final int LOOKING_FOR_OSC_COMMAND = 5;
   private static final int LOOKING_FOR_OSC_COMMAND_END = 6;
   private static final int LOOKING_FOR_OSC_PARAM = 7;
   private static final int LOOKING_FOR_ST = 8;
   private static final int LOOKING_FOR_CHARSET = 9;
   private static final int FIRST_ESC_CHAR = 27;
   private static final int SECOND_ESC_CHAR = 91;
   private static final int SECOND_OSC_CHAR = 93;
   private static final int BEL = 7;
   private static final int SECOND_ST_CHAR = 92;
   private static final int SECOND_CHARSET0_CHAR = 40;
   private static final int SECOND_CHARSET1_CHAR = 41;
   private AnsiProcessor ap;
   private static final int MAX_ESCAPE_SEQUENCE_LENGTH = 100;
   private final byte[] buffer = new byte[100];
   private int pos = 0;
   private int startOfValue;
   private final ArrayList<Object> options = new ArrayList<>();
   private int state = 0;
   private final Charset cs;
   private final AnsiOutputStream.WidthSupplier width;
   private final AnsiProcessor processor;
   private final AnsiType type;
   private final AnsiColors colors;
   private final AnsiOutputStream.IoRunnable installer;
   private final AnsiOutputStream.IoRunnable uninstaller;
   private AnsiMode mode;
   private boolean resetAtUninstall;

   public AnsiOutputStream(
      OutputStream var1,
      AnsiOutputStream.WidthSupplier var2,
      AnsiMode var3,
      AnsiProcessor var4,
      AnsiType var5,
      AnsiColors var6,
      Charset var7,
      AnsiOutputStream.IoRunnable var8,
      AnsiOutputStream.IoRunnable var9,
      boolean var10
   ) {
      super(var1);
      this.width = var2;
      this.processor = var4;
      this.type = var5;
      this.colors = var6;
      this.installer = var8;
      this.uninstaller = var9;
      this.resetAtUninstall = var10;
      this.cs = var7;
      this.setMode(var3);
   }

   public int getTerminalWidth() {
      return this.width.getTerminalWidth();
   }

   public AnsiType getType() {
      return this.type;
   }

   public AnsiColors getColors() {
      return this.colors;
   }

   public AnsiMode getMode() {
      return this.mode;
   }

   public void setMode(AnsiMode var1) {
      this.ap = var1 == AnsiMode.Strip
         ? new AnsiProcessor(this.out)
         : (var1 != AnsiMode.Force && this.processor != null ? this.processor : new ColorsAnsiProcessor(this.out, this.colors));
      this.mode = var1;
   }

   public boolean isResetAtUninstall() {
      return this.resetAtUninstall;
   }

   public void setResetAtUninstall(boolean var1) {
      this.resetAtUninstall = var1;
   }

   @Override
   public void write(int var1) {
      switch (this.state) {
         case 0:
            if (var1 == 27) {
               this.buffer[this.pos++] = (byte)var1;
               this.state = 1;
            } else {
               this.out.write(var1);
            }
            break;
         case 1:
            this.buffer[this.pos++] = (byte)var1;
            if (var1 == 91) {
               this.state = 2;
            } else if (var1 == 93) {
               this.state = 5;
            } else if (var1 == 40) {
               this.options.add(0);
               this.state = 9;
            } else if (var1 == 41) {
               this.options.add(1);
               this.state = 9;
            } else {
               this.reset(false);
            }
            break;
         case 2:
            this.buffer[this.pos++] = (byte)var1;
            if (34 == var1) {
               this.startOfValue = this.pos - 1;
               this.state = 3;
            } else if (48 <= var1 && var1 <= 57) {
               this.startOfValue = this.pos - 1;
               this.state = 4;
            } else if (59 == var1) {
               this.options.add(null);
            } else if (63 == var1) {
               this.options.add('?');
            } else if (61 == var1) {
               this.options.add('=');
            } else {
               this.processEscapeCommand(var1);
            }
            break;
         case 3:
            this.buffer[this.pos++] = (byte)var1;
            if (34 != var1) {
               String var7 = new String(this.buffer, this.startOfValue, this.pos - 1 - this.startOfValue, this.cs);
               this.options.add(var7);
               if (var1 == 59) {
                  this.state = 2;
               } else {
                  this.processEscapeCommand(var1);
               }
            }
            break;
         case 4:
            this.buffer[this.pos++] = (byte)var1;
            if (48 > var1 || var1 > 57) {
               String var6 = new String(this.buffer, this.startOfValue, this.pos - 1 - this.startOfValue);
               Integer var8 = Integer.valueOf(var6);
               this.options.add(var8);
               if (var1 == 59) {
                  this.state = 2;
               } else {
                  this.processEscapeCommand(var1);
               }
            }
            break;
         case 5:
            this.buffer[this.pos++] = (byte)var1;
            if (48 <= var1 && var1 <= 57) {
               this.startOfValue = this.pos - 1;
               this.state = 6;
            } else {
               this.reset(false);
            }
            break;
         case 6:
            this.buffer[this.pos++] = (byte)var1;
            if (59 == var1) {
               String var5 = new String(this.buffer, this.startOfValue, this.pos - 1 - this.startOfValue);
               Integer var3 = Integer.valueOf(var5);
               this.options.add(var3);
               this.startOfValue = this.pos;
               this.state = 7;
            } else if (48 > var1 || var1 > 57) {
               this.reset(false);
            }
            break;
         case 7:
            this.buffer[this.pos++] = (byte)var1;
            if (7 == var1) {
               String var4 = new String(this.buffer, this.startOfValue, this.pos - 1 - this.startOfValue, this.cs);
               this.options.add(var4);
               this.processOperatingSystemCommand();
            } else if (27 == var1) {
               this.state = 8;
            }
            break;
         case 8:
            this.buffer[this.pos++] = (byte)var1;
            if (92 == var1) {
               String var2 = new String(this.buffer, this.startOfValue, this.pos - 2 - this.startOfValue, this.cs);
               this.options.add(var2);
               this.processOperatingSystemCommand();
            } else {
               this.state = 7;
            }
            break;
         case 9:
            this.options.add((char)var1);
            this.processCharsetSelect();
      }

      if (this.pos >= this.buffer.length) {
         this.reset(false);
      }
   }

   private void processCharsetSelect() {
      try {
         this.reset(this.ap != null && this.ap.processCharsetSelect(this.options));
      } catch (RuntimeException var2) {
         this.reset(true);
         throw var2;
      }
   }

   private void processOperatingSystemCommand() {
      try {
         this.reset(this.ap != null && this.ap.processOperatingSystemCommand(this.options));
      } catch (RuntimeException var2) {
         this.reset(true);
         throw var2;
      }
   }

   private void processEscapeCommand(int var1) {
      try {
         this.reset(this.ap != null && this.ap.processEscapeCommand(this.options, var1));
      } catch (RuntimeException var3) {
         this.reset(true);
         throw var3;
      }
   }

   private void reset(boolean var1) {
      if (!var1) {
         this.out.write(this.buffer, 0, this.pos);
      }

      this.pos = 0;
      this.startOfValue = 0;
      this.options.clear();
      this.state = 0;
   }

   public void install() {
      if (this.installer != null) {
         this.installer.run();
      }
   }

   public void uninstall() {
      if (this.resetAtUninstall && this.type != AnsiType.Redirected && this.type != AnsiType.Unsupported) {
         this.setMode(AnsiMode.Default);
         this.write(RESET_CODE);
         this.flush();
      }

      if (this.uninstaller != null) {
         this.uninstaller.run();
      }
   }

   @Override
   public void close() {
      this.uninstall();
      super.close();
   }

   public interface IoRunnable {
      void run();
   }

   public interface WidthSupplier {
      int getTerminalWidth();
   }

   public static class ZeroWidthSupplier implements AnsiOutputStream.WidthSupplier {
      @Override
      public int getTerminalWidth() {
         return 0;
      }
   }
}
