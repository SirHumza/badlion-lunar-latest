...
public class ObfuscationManager implements IObfuscationManager {
   ...
   public void init() {
      if (!this.initDone) {
         ...
         IMixinAnnotationProcessor.CompilerEnvironment var3 = this.ap.getCompilerEnvironment();
         if (this.environments.size() == 0 && var3.isDevelopmentEnvironment()) {
            IMessagerEx.MessageType.setPrefix("(Mixin AP) ");
            this.ap
               .printMessage(
                  IMessagerEx.MessageType.NOTE,
                  "No obfuscation data are available and an IDE (" + var3.getFriendlyName() + ") was detected, quenching error levels "
               );
   ...
   public void writeMappings() {
      for (ObfuscationEnvironment var2 : this.environments) {
