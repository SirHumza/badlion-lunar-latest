...
public abstract class ZipUtil {
   ...
   public static byte[] toDosTime(long var0) {
      ...
      long var5 = var3 - 1980 << 25 | var4 << 21 | var2.get(5) << 16 | var2.get(11) << 11 | var2.get(12) << 5 | var2.get(13) >> 1;
      return ZipLong.getBytes(var5);
   }

   public static long adjustToLong(int var0) {
      return var0 < 0 ? 4294967296L + var0 : var0;
   }
   ...
   static boolean canHandleEntryData(ZipArchiveEntry var0) {
      return supportsEncryptionOf(var0) && supportsMethodOf(var0);
   }

   private static boolean supportsEncryptionOf(ZipArchiveEntry var0) {
      return !var0.getGeneralPurposeBit().usesEncryption();
   }

   private static boolean supportsMethodOf(ZipArchiveEntry var0) {
      return var0.getMethod() == 0
         || var0.getMethod() == ZipMethod.UNSHRINKING.getCode()
         || var0.getMethod() == ZipMethod.IMPLODING.getCode()
         || var0.getMethod() == 8;
   }

   static void checkRequestedFeatures(ZipArchiveEntry var0) {
      if (!supportsEncryptionOf(var0)) {
         throw new UnsupportedZipFeatureException(UnsupportedZipFeatureException.Feature.ENCRYPTION, var0);
      }

      if (!supportsMethodOf(var0)) {
