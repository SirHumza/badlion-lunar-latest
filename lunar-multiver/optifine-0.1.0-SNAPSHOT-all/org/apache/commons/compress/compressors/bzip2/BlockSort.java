package org.apache.commons.compress.compressors.bzip2;

import java.util.BitSet;

class BlockSort {
   private static final int QSORT_STACK_SIZE = 1000;
   private static final int FALLBACK_QSORT_STACK_SIZE = 100;
   private static final int STACK_SIZE = 1000;
   private int workDone;
   private int workLimit;
   private boolean firstAttempt;
   private final int[] stack_ll = new int[1000];
   private final int[] stack_hh = new int[1000];
   private final int[] stack_dd = new int[1000];
   private final int[] mainSort_runningOrder = new int[256];
   private final int[] mainSort_copy = new int[256];
   private final boolean[] mainSort_bigDone = new boolean[256];
   private final int[] ftab = new int[65537];
   private final char[] quadrant;
   private static final int FALLBACK_QSORT_SMALL_THRESH = 10;
   private int[] eclass;
   private static final int[] INCS = new int[]{1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524, 88573, 265720, 797161, 2391484};
   private static final int SMALL_THRESH = 20;
   private static final int DEPTH_THRESH = 10;
   private static final int WORK_FACTOR = 30;
   private static final int SETMASK = 2097152;
   private static final int CLEARMASK = -2097153;

   BlockSort(BZip2CompressorOutputStream.Data var1) {
      this.quadrant = var1.sfmap;
   }

   void blockSort(BZip2CompressorOutputStream.Data var1, int var2) {
      this.workLimit = 30 * var2;
      this.workDone = 0;
      this.firstAttempt = true;
      if (var2 + 1 < 10000) {
         this.fallbackSort(var1, var2);
      } else {
         this.mainSort(var1, var2);
         if (this.firstAttempt && this.workDone > this.workLimit) {
            this.fallbackSort(var1, var2);
         }
      }

      int[] var3 = var1.fmap;
      var1.origPtr = -1;

      for (int var4 = 0; var4 <= var2; var4++) {
         if (var3[var4] == 0) {
            var1.origPtr = var4;
            break;
         }
      }
   }

   final void fallbackSort(BZip2CompressorOutputStream.Data var1, int var2) {
      var1.block[0] = var1.block[var2 + 1];
      this.fallbackSort(var1.fmap, var1.block, var2 + 1);

      for (int var3 = 0; var3 < var2 + 1; var3++) {
         var1.fmap[var3]--;
      }

      for (int var4 = 0; var4 < var2 + 1; var4++) {
         if (var1.fmap[var4] == -1) {
            var1.fmap[var4] = var2;
            break;
         }
      }
   }

   private void fallbackSimpleSort(int[] var1, int[] var2, int var3, int var4) {
      if (var3 != var4) {
         if (var4 - var3 > 3) {
            for (int var6 = var4 - 4; var6 >= var3; var6--) {
               int var7 = var1[var6];
               int var8 = var2[var7];

               int var5;
               for (var5 = var6 + 4; var5 <= var4 && var8 > var2[var1[var5]]; var5 += 4) {
                  var1[var5 - 4] = var1[var5];
               }

               var1[var5 - 4] = var7;
            }
         }

         for (int var10 = var4 - 1; var10 >= var3; var10--) {
            int var11 = var1[var10];
            int var12 = var2[var11];

            int var9;
            for (var9 = var10 + 1; var9 <= var4 && var12 > var2[var1[var9]]; var9++) {
               var1[var9 - 1] = var1[var9];
            }

            var1[var9 - 1] = var11;
         }
      }
   }

   private void fswap(int[] var1, int var2, int var3) {
      int var4 = var1[var2];
      var1[var2] = var1[var3];
      var1[var3] = var4;
   }

   private void fvswap(int[] var1, int var2, int var3, int var4) {
      while (var4 > 0) {
         this.fswap(var1, var2, var3);
         var2++;
         var3++;
         var4--;
      }
   }

   private int fmin(int var1, int var2) {
      return var1 < var2 ? var1 : var2;
   }

   private void fpush(int var1, int var2, int var3) {
      this.stack_ll[var1] = var2;
      this.stack_hh[var1] = var3;
   }

   private int[] fpop(int var1) {
      return new int[]{this.stack_ll[var1], this.stack_hh[var1]};
   }

   private void fallbackQSort3(int[] var1, int[] var2, int var3, int var4) {
      long var12 = 0L;
      int var14 = 0;
      this.fpush(var14++, var3, var4);

      label69:
      while (var14 > 0) {
         int[] var15 = this.fpop(--var14);
         int var5 = var15[0];
         int var8 = var15[1];
         if (var8 - var5 < 10) {
            this.fallbackSimpleSort(var1, var2, var5, var8);
         } else {
            var12 = (var12 * 7621L + 1L) % 32768L;
            long var16 = var12 % 3L;
            long var18;
            if (var16 == 0L) {
               var18 = var2[var1[var5]];
            } else if (var16 == 1L) {
               var18 = var2[var1[var5 + var8 >>> 1]];
            } else {
               var18 = var2[var1[var8]];
            }

            int var7 = var5;
            int var6 = var5;
            int var10 = var8;
            int var9 = var8;

            while (true) {
               if (var6 <= var9) {
                  int var11 = var2[var1[var6]] - (int)var18;
                  if (var11 == 0) {
                     this.fswap(var1, var6, var7);
                     var7++;
                     var6++;
                     continue;
                  }

                  if (var11 <= 0) {
                     var6++;
                     continue;
                  }
               }

               while (true) {
                  if (var6 <= var9) {
                     int var21 = var2[var1[var9]] - (int)var18;
                     if (var21 == 0) {
                        this.fswap(var1, var9, var10);
                        var10--;
                        var9--;
                        continue;
                     }

                     if (var21 >= 0) {
                        var9--;
                        continue;
                     }
                  }

                  if (var6 > var9) {
                     if (var10 >= var7) {
                        int var22 = this.fmin(var7 - var5, var6 - var7);
                        this.fvswap(var1, var5, var6 - var22, var22);
                        int var20 = this.fmin(var8 - var10, var10 - var9);
                        this.fvswap(var1, var9 + 1, var8 - var20 + 1, var20);
                        var22 = var5 + var6 - var7 - 1;
                        var20 = var8 - (var10 - var9) + 1;
                        if (var22 - var5 > var8 - var20) {
                           this.fpush(var14++, var5, var22);
                           this.fpush(var14++, var20, var8);
                        } else {
                           this.fpush(var14++, var20, var8);
                           this.fpush(var14++, var5, var22);
                        }
                     }
                     continue label69;
                  }

                  this.fswap(var1, var6, var9);
                  var6++;
                  var9--;
                  break;
               }
            }
         }
      }
   }

   private int[] getEclass() {
      return this.eclass == null ? (this.eclass = new int[this.quadrant.length / 2]) : this.eclass;
   }

   final void fallbackSort(int[] var1, byte[] var2, int var3) {
      int[] var4 = new int[257];
      int[] var15 = this.getEclass();

      for (int var6 = 0; var6 < var3; var6++) {
         var15[var6] = 0;
      }

      for (int var17 = 0; var17 < var3; var17++) {
         var4[var2[var17] & 0xFF]++;
      }

      for (int var18 = 1; var18 < 257; var18++) {
         var4[var18] += var4[var18 - 1];
      }

      int var19 = 0;

      while (var19 < var3) {
         int var7 = var2[var19] & 255;
         int var8 = var4[var7] - 1;
         var4[var7] = var8;
         var1[var8] = var19++;
      }

      int var14 = 64 + var3;
      BitSet var16 = new BitSet(var14);

      for (int var20 = 0; var20 < 256; var20++) {
         var16.set(var4[var20]);
      }

      for (int var21 = 0; var21 < 32; var21++) {
         var16.set(var3 + 2 * var21);
         var16.clear(var3 + 2 * var21 + 1);
      }

      byte var5 = 1;

      int var13;
      do {
         int var24 = 0;

         for (int var22 = 0; var22 < var3; var22++) {
            if (var16.get(var22)) {
               var24 = var22;
            }

            int var25 = var1[var22] - var5;
            if (var25 < 0) {
               var25 += var3;
            }

            var15[var25] = var24;
         }

         var13 = 0;
         int var10 = -1;

         while (true) {
            int var26 = var10 + 1;
            var26 = var16.nextClearBit(var26);
            int var9 = var26 - 1;
            if (var9 >= var3) {
               break;
            }

            var26 = var16.nextSetBit(var26 + 1);
            var10 = var26 - 1;
            if (var10 >= var3) {
               break;
            }

            if (var10 > var9) {
               var13 += var10 - var9 + 1;
               this.fallbackQSort3(var1, var15, var9, var10);
               int var11 = -1;

               for (int var23 = var9; var23 <= var10; var23++) {
                  int var12 = var15[var1[var23]];
                  if (var11 != var12) {
                     var16.set(var23);
                     var11 = var12;
                  }
               }
            }
         }

         var5 *= 2;
      } while (var5 <= var3 && var13 != 0);
   }

   private boolean mainSimpleSort(BZip2CompressorOutputStream.Data var1, int var2, int var3, int var4, int var5) {
      int var6 = var3 - var2 + 1;
      if (var6 < 2) {
         return this.firstAttempt && this.workDone > this.workLimit;
      }

      int var7 = 0;

      while (INCS[var7] < var6) {
         var7++;
      }

      int[] var8 = var1.fmap;
      char[] var9 = this.quadrant;
      byte[] var10 = var1.block;
      int var11 = var5 + 1;
      boolean var12 = this.firstAttempt;
      int var13 = this.workLimit;
      int var14 = this.workDone;

      label193:
      while (--var7 >= 0) {
         int var15 = INCS[var7];
         int var16 = var2 + var15 - 1;
         int var17 = var2 + var15;

         while (var17 <= var3) {
            for (int var18 = 3; var17 <= var3 && --var18 >= 0; var17++) {
               int var19 = var8[var17];
               int var20 = var19 + var4;
               int var21 = var17;
               boolean var22 = false;
               int var23 = 0;

               label185:
               while (true) {
                  if (var22) {
                     var8[var21] = var23;
                     if ((var21 -= var15) <= var16) {
                        break;
                     }
                  } else {
                     var22 = true;
                  }

                  var23 = var8[var21 - var15];
                  int var24 = var23 + var4;
                  int var25 = var20;
                  if (var10[var24 + 1] == var10[var25 + 1]) {
                     if (var10[var24 + 2] == var10[var25 + 2]) {
                        if (var10[var24 + 3] == var10[var25 + 3]) {
                           if (var10[var24 + 4] == var10[var25 + 4]) {
                              if (var10[var24 + 5] == var10[var25 + 5]) {
                                 var24 += 6;
                                 byte var10000 = var10[var24];
                                 var25 += 6;
                                 if (var10000 == var10[var25]) {
                                    int var26 = var5;

                                    while (true) {
                                       if (var26 <= 0) {
                                          break label185;
                                       }

                                       var26 -= 4;
                                       if (var10[var24 + 1] != var10[var25 + 1]) {
                                          if ((var10[var24 + 1] & 255) <= (var10[var25 + 1] & 255)) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var9[var24] != var9[var25]) {
                                          if (var9[var24] <= var9[var25]) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var10[var24 + 2] != var10[var25 + 2]) {
                                          if ((var10[var24 + 2] & 255) <= (var10[var25 + 2] & 255)) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var9[var24 + 1] != var9[var25 + 1]) {
                                          if (var9[var24 + 1] <= var9[var25 + 1]) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var10[var24 + 3] != var10[var25 + 3]) {
                                          if ((var10[var24 + 3] & 255) <= (var10[var25 + 3] & 255)) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var9[var24 + 2] != var9[var25 + 2]) {
                                          if (var9[var24 + 2] <= var9[var25 + 2]) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var10[var24 + 4] != var10[var25 + 4]) {
                                          if ((var10[var24 + 4] & 255) <= (var10[var25 + 4] & 255)) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       if (var9[var24 + 3] != var9[var25 + 3]) {
                                          if (var9[var24 + 3] <= var9[var25 + 3]) {
                                             break label185;
                                          }
                                          break;
                                       }

                                       var24 += 4;
                                       if (var24 >= var11) {
                                          var24 -= var11;
                                       }

                                       var25 += 4;
                                       if (var25 >= var11) {
                                          var25 -= var11;
                                       }

                                       var14++;
                                    }
                                 } else if ((var10[var24] & 255) <= (var10[var25] & 255)) {
                                    break;
                                 }
                              } else if ((var10[var24 + 5] & 255) <= (var10[var25 + 5] & 255)) {
                                 break;
                              }
                           } else if ((var10[var24 + 4] & 255) <= (var10[var25 + 4] & 255)) {
                              break;
                           }
                        } else if ((var10[var24 + 3] & 255) <= (var10[var25 + 3] & 255)) {
                           break;
                        }
                     } else if ((var10[var24 + 2] & 255) <= (var10[var25 + 2] & 255)) {
                        break;
                     }
                  } else if ((var10[var24 + 1] & 255) <= (var10[var25 + 1] & 255)) {
                     break;
                  }
               }

               var8[var21] = var19;
            }

            if (var12 && var17 <= var3 && var14 > var13) {
               break label193;
            }
         }
      }

      this.workDone = var14;
      return var12 && var14 > var13;
   }

   private static void vswap(int[] var0, int var1, int var2, int var3) {
      var3 += var1;

      while (var1 < var3) {
         int var4 = var0[var1];
         var0[var1++] = var0[var2];
         var0[var2++] = var4;
      }
   }

   private static byte med3(byte var0, byte var1, byte var2) {
      return var0 < var1 ? (var1 < var2 ? var1 : (var0 < var2 ? var2 : var0)) : (var1 > var2 ? var1 : (var0 > var2 ? var2 : var0));
   }

   private void mainQSort3(BZip2CompressorOutputStream.Data var1, int var2, int var3, int var4, int var5) {
      int[] var6 = this.stack_ll;
      int[] var7 = this.stack_hh;
      int[] var8 = this.stack_dd;
      int[] var9 = var1.fmap;
      byte[] var10 = var1.block;
      var6[0] = var2;
      var7[0] = var3;
      var8[0] = var4;
      int var11 = 1;

      label71:
      while (--var11 >= 0) {
         int var12 = var6[var11];
         int var13 = var7[var11];
         int var14 = var8[var11];
         if (var13 - var12 >= 20 && var14 <= 10) {
            int var15 = var14 + 1;
            int var16 = med3(var10[var9[var12] + var15], var10[var9[var13] + var15], var10[var9[var12 + var13 >>> 1] + var15]) & 255;
            int var17 = var12;
            int var18 = var13;
            int var19 = var12;
            int var20 = var13;

            while (true) {
               if (var17 <= var18) {
                  int var21 = (var10[var9[var17] + var15] & 255) - var16;
                  if (var21 == 0) {
                     int var31 = var9[var17];
                     var9[var17++] = var9[var19];
                     var9[var19++] = var31;
                     continue;
                  }

                  if (var21 < 0) {
                     var17++;
                     continue;
                  }
               }

               while (true) {
                  if (var17 <= var18) {
                     int var25 = (var10[var9[var18] + var15] & 255) - var16;
                     if (var25 == 0) {
                        int var30 = var9[var18];
                        var9[var18--] = var9[var20];
                        var9[var20--] = var30;
                        continue;
                     }

                     if (var25 > 0) {
                        var18--;
                        continue;
                     }
                  }

                  if (var17 > var18) {
                     if (var20 < var19) {
                        var6[var11] = var12;
                        var7[var11] = var13;
                        var8[var11] = var15;
                        var11++;
                     } else {
                        int var27 = var19 - var12 < var17 - var19 ? var19 - var12 : var17 - var19;
                        vswap(var9, var12, var17 - var27, var27);
                        int var22 = var13 - var20 < var20 - var18 ? var13 - var20 : var20 - var18;
                        vswap(var9, var17, var13 - var22 + 1, var22);
                        var27 = var12 + var17 - var19 - 1;
                        var22 = var13 - (var20 - var18) + 1;
                        var6[var11] = var12;
                        var7[var11] = var27;
                        var8[var11] = var14;
                        var6[++var11] = var27 + 1;
                        var7[var11] = var22 - 1;
                        var8[var11] = var15;
                        var6[++var11] = var22;
                        var7[var11] = var13;
                        var8[var11] = var14;
                        var11++;
                     }
                     continue label71;
                  }

                  int var26 = var9[var17];
                  var9[var17++] = var9[var18];
                  var9[var18--] = var26;
                  break;
               }
            }
         } else if (this.mainSimpleSort(var1, var12, var13, var14, var5)) {
            return;
         }
      }
   }

   final void mainSort(BZip2CompressorOutputStream.Data var1, int var2) {
      int[] var3 = this.mainSort_runningOrder;
      int[] var4 = this.mainSort_copy;
      boolean[] var5 = this.mainSort_bigDone;
      int[] var6 = this.ftab;
      byte[] var7 = var1.block;
      int[] var8 = var1.fmap;
      char[] var9 = this.quadrant;
      int var10 = this.workLimit;
      boolean var11 = this.firstAttempt;
      int var12 = 65537;

      while (--var12 >= 0) {
         var6[var12] = 0;
      }

      for (int var21 = 0; var21 < 20; var21++) {
         var7[var2 + var21 + 2] = var7[var21 % (var2 + 1) + 1];
      }

      var12 = var2 + 20 + 1;

      while (--var12 >= 0) {
         var9[var12] = 0;
      }

      var7[0] = var7[var2 + 1];
      var12 = var7[0] & 255;

      for (int var13 = 0; var13 <= var2; var13++) {
         int var14 = var7[var13 + 1] & 255;
         var6[(var12 << 8) + var14]++;
         var12 = var14;
      }

      for (int var26 = 1; var26 <= 65536; var26++) {
         var6[var26] += var6[var26 - 1];
      }

      var12 = var7[1] & 255;

      for (int var27 = 0; var27 < var2; var27++) {
         int var31 = var7[var27 + 2] & 255;
         var8[--var6[(var12 << 8) + var31]] = var27;
         var12 = var31;
      }

      var8[--var6[((var7[var2 + 1] & 255) << 8) + (var7[1] & 255)]] = var2;

      for (int var28 = 256; --var28 >= 0; var3[var28] = var28) {
         var5[var28] = false;
      }

      short var29 = 364;

      while (var29 != 1) {
         var29 /= 3;

         for (int var32 = var29; var32 <= 255; var32++) {
            int var15 = var3[var32];
            int var16 = var6[var15 + 1 << 8] - var6[var15 << 8];
            int var17 = var29 - 1;
            int var18 = var32;

            for (int var19 = var3[var18 - var29]; var6[var19 + 1 << 8] - var6[var19 << 8] > var16; var19 = var3[var18 - var29]) {
               var3[var18] = var19;
               var18 -= var29;
               if (var18 <= var17) {
                  break;
               }
            }

            var3[var18] = var15;
         }
      }

      for (int var30 = 0; var30 <= 255; var30++) {
         int var33 = var3[var30];

         for (int var34 = 0; var34 <= 255; var34++) {
            int var39 = (var33 << 8) + var34;
            int var42 = var6[var39];
            if ((var42 & 2097152) != 2097152) {
               int var45 = var42 & -2097153;
               int var47 = (var6[var39 + 1] & -2097153) - 1;
               if (var47 > var45) {
                  this.mainQSort3(var1, var45, var47, 2, var2);
                  if (var11 && this.workDone > var10) {
                     return;
                  }
               }

               var6[var39] = var42 | 2097152;
            }
         }

         for (int var35 = 0; var35 <= 255; var35++) {
            var4[var35] = var6[(var35 << 8) + var33] & -2097153;
         }

         int var36 = var6[var33 << 8] & -2097153;

         for (int var40 = var6[var33 + 1 << 8] & -2097153; var36 < var40; var36++) {
            int var43 = var8[var36];
            var12 = var7[var43] & 255;
            if (!var5[var12]) {
               var8[var4[var12]] = var43 == 0 ? var2 : var43 - 1;
               var4[var12]++;
            }
         }

         var36 = 256;

         while (--var36 >= 0) {
            var6[(var36 << 8) + var33] = var6[(var36 << 8) + var33] | 2097152;
         }

         var5[var33] = true;
         if (var30 < 255) {
            var36 = var6[var33 << 8] & -2097153;
            int var41 = (var6[var33 + 1 << 8] & -2097153) - var36;
            int var44 = 0;

            while (var41 >> var44 > 65534) {
               var44++;
            }

            for (int var46 = 0; var46 < var41; var46++) {
               int var48 = var8[var36 + var46];
               char var20 = (char)(var46 >> var44);
               var9[var48] = var20;
               if (var48 < 20) {
                  var9[var48 + var2 + 1] = var20;
               }
            }
         }
      }
   }
}
