...
public class ShadersTex {
   ...
   public static void uploadTexSubForLoadAtlas(
      bmh textureMap, String iconName, int[][] data, int width, int height, int xoffset, int yoffset, boolean linear, boolean clamp
   ) {
      MultiTexID updatingTex = textureMap.multiTex;
   ...
   public static int[][] readImageAndMipmaps(bmh updatingTextureMap, String name, int width, int height, int numLevels, boolean border, int defColor) {
      MultiTexID updatingTex = updatingTextureMap.multiTex;
      int[][] aaint = new int[numLevels][];
      int[] aint;
      aaint[0] = aint = new int[width * height];
      boolean goodImage = false;
      BufferedImage image = readImage(updatingTextureMap.completeResourceLocation(new jy(name)));
      if (image != null) {
   ...
   public static int loadSimpleTexture(
      int textureID, BufferedImage bufferedimage, boolean linear, boolean clamp, bni resourceManager, jy location, MultiTexID multiTex
   ) {
