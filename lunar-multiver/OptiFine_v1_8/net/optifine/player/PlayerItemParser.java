...
public class PlayerItemParser {
   ...
   private static jy makeResourceLocation(String texture) {
      int pos = texture.indexOf(58);
      if (pos < 0) {
         return new jy(texture);
      }

      String domain = texture.substring(0, pos);
      String path = texture.substring(pos + 1);
      return new jy(domain, path);
   }
   ...
   public static PlayerItemRenderer parseItemRenderer(JsonObject elem, Dimension textureDim) {
      ...
      if (!Config.equals(type, "ModelBox")) {
         ...
         bct mr = parseModelRenderer(elem, modelBase, null, null);
         return new PlayerItemRenderer(attachTo, mr);
      }
   }

   public static bct parseModelRenderer(JsonObject elem, bbo modelBase, int[] parentTextureSize, String basePath) {
      bct mr = new bct(modelBase);
      String id = Json.getString(elem, "id");
      mr.setId(id);
      ...
      for (int i = 0; i < rotateAngles.length; i++) {
         rotateAngles[i] = rotateAngles[i] / 180.0F * ns.PI;
      }
      ...
      if (submodel != null) {
         bct subMr = parseModelRenderer(submodel, modelBase, textureSize, basePath);
         mr.a(subMr);
      }
      ...
      if (submodels != null) {
         for (int i = 0; i < submodels.size(); i++) {
            ...
            bct subMr = parseModelRenderer(sm, modelBase, textureSize, basePath);
            if (subMr.getId() != null) {
               bct subMrId = mr.getChild(subMr.getId());
               if (subMrId != null) {
