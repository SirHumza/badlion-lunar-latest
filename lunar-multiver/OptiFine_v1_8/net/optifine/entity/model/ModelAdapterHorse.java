...
public class ModelAdapterHorse extends ModelAdapter {
   ...
   public bct getModelRenderer(bbo model, String modelPart) {
      if (!(model instanceof bbh)) {
         return null;
      } else {
         bbh modelHorse = (bbh)model;
         Map<String, Integer> mapParts = getMapPartFields();
         if (mapParts.containsKey(modelPart)) {
            int index = mapParts.get(modelPart);
            return (bct)Reflector.getFieldValue(modelHorse, Reflector.ModelHorse_ModelRenderers, index);
         } else {
            return null;
         }
      }
   ...
   public IEntityRenderer makeEntityRender(bbo modelBase, float shadowSize) {
      ...
      return new bjd(renderManager, (bbh)modelBase, shadowSize);
   }
}
