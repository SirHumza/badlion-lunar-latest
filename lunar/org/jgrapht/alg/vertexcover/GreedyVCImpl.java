...
public class GreedyVCImpl<V, E> implements VertexCoverAlgorithm<V> {
   ...
   public GreedyVCImpl(Graph<V, E> var1) {
   ...
   public GreedyVCImpl(Graph<V, E> var1, Map<V, Double> var2) {
      this.graph = GraphTests.requireUndirected(var1);
   ...
   public VertexCoverAlgorithm.VertexCover<V> getVertexCover() {
      ...
      for (Object var6 : this.graph.edgeSet()) {
         ...
         assert var8.neighbors.get(var10) == var10.neighbors.get(var8) : " in an undirected graph, if vx is a neighbor of ux, then ux must be a neighbor of vx";
      }
      ...
      while (!var11.isEmpty()) {
         ...
         for (RatioVertex var14 : var12.neighbors.keySet()) {
            if (var14 != var12) {
               var11.remove(var14);
               var14.removeNeighbor(var12);
               if (var14.getDegree() > 0) {
                  var11.add(var14);
               }
            }
         }
         ...
         assert var11.parallelStream().noneMatch(var1x -> var1x.ID == var12.ID) : "vx should no longer exist in the working graph";
      }
