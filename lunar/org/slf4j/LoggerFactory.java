...
public final class LoggerFactory {
   ...
   private static ServiceLoader<SLF4JServiceProvider> getServiceLoader(ClassLoader var0) {
      ...
      if (var2 == null) {
         ...
         var1 = AccessController.doPrivileged(var3);
      }
   ...
   private static void reportIgnoredStaticLoggerBinders(Set<URL> var0) {
      if (!var0.isEmpty()) {
         ...
         for (URL var2 : var0) {
            Reporter.warn("Ignoring binding found at [" + var2 + "]");
         }
   ...
   static void failedBinding(Throwable var0) {
      ...
      Reporter.error("Failed to instantiate SLF4J LoggerFactory", var0);
   }
   ...
   private static void emitReplayWarning(int var0) {
      Reporter.warn("A number (" + var0 + ") of logging calls during the initialization phase have been intercepted and are");
   ...
   private static final void versionSanityCheck() {
      try {
         ...
         if (!var1) {
            Reporter.warn(
               "The requested version " + var0 + " by your slf4j provider is not compatible with " + Arrays.asList(API_COMPATIBILITY_LIST).toString()
            );
   ...
   private static void reportMultipleBindingAmbiguity(List<SLF4JServiceProvider> var0) {
      if (isAmbiguousProviderList(var0)) {
         ...
         for (SLF4JServiceProvider var2 : var0) {
            Reporter.warn("Found provider [" + var2 + "]");
         }
   ...
   private static void reportActualBinding(List<SLF4JServiceProvider> var0) {
      ...
      if (isAmbiguousProviderList(var0)) {
         Reporter.info("Actual provider is of type [" + var0.get(0) + "]");
      } else {
         SLF4JServiceProvider var1 = (SLF4JServiceProvider)var0.get(0);
         Reporter.debug("Connected with provider of type [" + var1.getClass().getName() + "]");
      }
   }
   ...
   public static Logger getLogger(Class<?> var0) {
      ...
      if (DETECT_LOGGER_NAME_MISMATCH) {
         ...
         if (var2 != null && nonMatchingClasses(var0, var2)) {
            Reporter.warn(String.format("Detected logger name mismatch. Given name: \"%s\"; computed name: \"%s\".", var1.getName(), var2.getName()));
