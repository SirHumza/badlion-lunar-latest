...
public interface DynamicOps<T> {
   ...
   default Number getNumberValue(T var1, Number var2) {
      return this.getNumberValue((T)var1).result().orElse(var2);
   }
   ...
   default DataResult<Boolean> getBooleanValue(T var1) {
      return this.getNumberValue((T)var1).map(var0 -> var0.byteValue() != 0);
   }
   ...
   default DataResult<T> mergeToList(T var1, List<T> var2) {
      ...
      for (Object var5 : var2) {
         var3 = var3.flatMap(var2x -> this.mergeToList((T)var2x, (T)var5));
      }
   ...
   default DataResult<T> mergeToMap(T var1, Map<T, T> var2) {
      return this.mergeToMap((T)var1, MapLike.forMap(var2, this));
   }

   default DataResult<T> mergeToMap(T var1, MapLike<T> var2) {
      AtomicReference var3 = new AtomicReference<>(DataResult.success(var1));
      var2.entries()
         .forEach(
            var2x -> var3.setPlain(((DataResult)var3.getPlain()).flatMap(var2xx -> this.mergeToMap((T)var2xx, (T)var2x.getFirst(), (T)var2x.getSecond())))
         );
      return (DataResult<T>)var3.getPlain();
   }

   default DataResult<T> mergeToPrimitive(T var1, T var2) {
      return !Objects.equals(var1, this.empty())
         ? DataResult.error(() -> "Do not know how to append a primitive value " + var2 + " to " + var1, (T)var2)
         : DataResult.success((T)var2);
   }

   DataResult<Stream<Pair<T, T>>> getMapValues(T var1);

   default DataResult<Consumer<BiConsumer<T, T>>> getMapEntries(T var1) {
      return this.getMapValues((T)var1).map(var0 -> var1x -> var0.forEach(var1xx -> var1x.accept(var1xx.getFirst(), var1xx.getSecond())));
   }

   T createMap(Stream<Pair<T, T>> var1);

   default DataResult<MapLike<T>> getMap(T var1) {
      return this.getMapValues((T)var1).flatMap(var1x -> {
         try {
            return DataResult.success(MapLike.forMap(var1x.collect(Pair.toMap()), this));
   ...
   default T createMap(Map<T, T> var1) {
      return this.createMap(var1.entrySet().stream().map(var0 -> Pair.of((T)var0.getKey(), (T)var0.getValue())));
   }

   DataResult<Stream<T>> getStream(T var1);

   default DataResult<Consumer<Consumer<T>>> getList(T var1) {
      return this.getStream((T)var1).map(var0 -> var0::forEach);
   }

   T createList(Stream<T> var1);

   default DataResult<ByteBuffer> getByteBuffer(T var1) {
      return this.getStream((T)var1).flatMap(var2 -> {
         List var3 = var2.collect(Collectors.toList());
         if (!var3.stream().allMatch(var1xx -> this.getNumberValue((T)var1xx).isSuccess())) {
         ...
         for (int var5 = 0; var5 < var3.size(); var5++) {
            var4.put(var5, this.getNumberValue((T)var3.get(var5)).result().get().byteValue());
         }
   ...
   default T createByteList(ByteBuffer var1) {
      return this.createList(IntStream.range(0, var1.capacity()).mapToObj(var2 -> this.createByte(var1.get(var2))));
   }

   default DataResult<IntStream> getIntStream(T var1) {
      return this.getStream((T)var1)
         .flatMap(
            var2 -> {
               List var3 = var2.toList();
               return var3.stream().allMatch(var1xx -> this.getNumberValue((T)var1xx).isSuccess())
                  ? DataResult.success(var3.stream().mapToInt(var1xx -> this.getNumberValue((T)var1xx).getOrThrow().intValue()))
      ...
      return this.getStream((T)var1)
         .flatMap(
            var2 -> {
               List var3 = var2.toList();
               return var3.stream().allMatch(var1xx -> this.getNumberValue((T)var1xx).isSuccess())
                  ? DataResult.success(var3.stream().mapToLong(var1xx -> this.getNumberValue((T)var1xx).getOrThrow().longValue()))
   ...
   default DataResult<T> get(T var1, String var2) {
      return this.getGeneric((T)var1, this.createString(var2));
   }

   default DataResult<T> getGeneric(T var1, T var2) {
      return this.getMap((T)var1)
         .flatMap(
            var2x -> Optional.ofNullable(var2x.get((T)var2))
               .map(DataResult::success)
               .orElseGet(() -> DataResult.error(() -> "No element " + var2 + " in the map " + var1))
         );
   }

   default T set(T var1, String var2, T var3) {
      return this.mergeToMap((T)var1, this.createString(var2), (T)var3).result().orElse((T)var1);
   }

   default T update(T var1, String var2, Function<T, T> var3) {
      return this.get((T)var1, var2).map(var4 -> this.set((T)var1, var2, (T)var3.apply((T)var4))).result().orElse((T)var1);
   }

   default T updateGeneric(T var1, T var2, Function<T, T> var3) {
      return this.getGeneric((T)var1, (T)var2).flatMap(var4 -> this.mergeToMap((T)var1, (T)var2, (T)var3.apply((T)var4))).result().orElse((T)var1);
   }
   ...
   default <E> Function<E, DataResult<T>> withEncoder(Encoder<E> var1) {
      return var2 -> var1.encodeStart(this, var2);
   }

   default <E> Function<T, DataResult<Pair<E, T>>> withDecoder(Decoder<E> var1) {
      return var2 -> var1.decode(this, var2);
   }

   default <E> Function<T, DataResult<E>> withParser(Decoder<E> var1) {
      return var2 -> var1.parse(this, var2);
   }

   default <U> U convertList(DynamicOps<U> var1, T var2) {
      return var1.createList(this.getStream((T)var2).result().orElse(Stream.empty()).map(var2x -> this.convertTo(var1, (T)var2x)));
   }

   default <U> U convertMap(DynamicOps<U> var1, T var2) {
      return var1.createMap(
         this.getMapValues((T)var2)
            .result()
            .orElse(Stream.empty())
            .map(var2x -> Pair.of(this.convertTo(var1, var2x.getFirst()), this.convertTo(var1, var2x.getSecond())))
      );
   }
}
